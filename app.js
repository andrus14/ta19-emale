...
fetch('https://cors-anywhere.herokuapp.com/http://andmebaas.stat.ee/sdmx-json/data/PA633/74+DBL113+DBL200+DBL245+DBL432.3.1/all?startTime=2014&endTime=2014&dimensionAtObservation=allDimensions')
.then(response => {
    ...
    submitBtn.addEventListener('click', event => {
        ...
        if ( selectedKey && hours ) {
            ...
            rowDiv.querySelector('.list-item-delete').addEventListener('click', event => {
                const salary = event.currentTarget.getAttribute('data-salary-float')
                total -= salary
